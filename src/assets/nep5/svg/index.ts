import NEO from './NEO.svg'
import NEO2 from './NEO2.svg'
import GAS from './GAS.svg'
import GAS2 from './GAS2.svg'
import ACAT from './ACAT.svg'
import APH from './APH.svg'
import APT from './APT.svg'
import ASA from './ASA.svg'
import AVA from './AVA.svg'
import BDN from './BDN.svg'
import CGAS from './CGAS.svg'
import COUP from './COUP.svg'
import CPX from './CPX.svg'
import DAD from './DAD.svg'
import DBC from './DBC.svg'
import EDS from './EDS.svg'
import EFX from './EFX.svg'
import FTW from './FTW.svg'
import GALA from './GALA.svg'
import GTA from './GTA.svg'
import GUARD from './GUARD.svg'
import LFX from './LFX.svg'
import LRN from './LRN.svg'
import LX from './LX.svg'
import MCT from './MCT.svg'
import NEX from './NEX.svg'
import NKN from './NKN.svg'
import NNC from './NNC.svg'
import NNN from './NNN.svg'
import NRVE from './NRVE.svg'
import NVL from './NVL.svg'
import OBT from './OBT.svg'
import ONT from './ONT.svg'
import PHX from './PHX.svg'
import PKC from './PKC.svg'
import QLC from './QLC.svg'
import RHT from './RHT.svg'
import SCC from './SCC.svg'
import SDS from './SDS.svg'
import SENNO from './SENNO.svg'
import SOUL from './SOUL.svg'
import SWTH from './SWTH.svg'
import THOR from './THOR.svg'
import TKY from './TKY.svg'
import TNC from './TNC.svg'
import TOLL from './TOLL.svg'
import UTD from './UTD.svg'
import WWB from './WWB.svg'
import XQTA from './XQTA.svg'
import ZPT from './ZPT.svg'
import NOS from './NOS.svg'
import FLM from './FLM.svg'
import EPN from './EPN.svg'
import EXT from './EXT.svg'
import GHOST from './GHOST.svg'
import GM from './GM.svg'
import GFUND from './GFUND.svg'

export type TokenMap = { [key: string]: string }

const tokens: TokenMap = {
  NEO,
  NEO2,
  GAS,
  GAS2,
  ACAT,
  APH,
  APT,
  ASA,
  AVA,
  BDN,
  CGAS,
  COUP,
  CPX,
  DAD,
  DBC,
  EDS,
  EFX,
  FTW,
  GALA,
  GTA,
  GUARD,
  LFX,
  LRN,
  LX,
  MCT,
  NEX,
  NKN,
  NNC,
  NNN,
  NRVE,
  NVL,
  OBT,
  ONT,
  PHX,
  PKC,
  QLC,
  RHT,
  SCC,
  SDS,
  SENNO,
  SOUL,
  SWTH,
  THOR,
  TKY,
  TNC,
  TOLL,
  UTD,
  WWB,
  XQTA,
  ZPT,
  NOS,
  FLM,
  EPN,
  EXT,
  GHOST,
  GM,
  GFUND,
}

export default tokens
