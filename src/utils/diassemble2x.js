...
export function disassemble(text_script) {
    let out = ''
    const script = Buffer.from(text_script, 'hex')
        ...
        if (opcodetable.hasOwnProperty(opcode)) {
            ...
            if (opcodedata.name === 'SYSCALL') {
                if (opcodedata.length == 4) {
                    ...
                    ip += 1
                    let method_length = script.readUInt8(ip)
                    ip += 1
                    let data = script.slice(ip, ip+method_length)
                    let datawords = CryptoJS.enc.Hex.parse(data.toString('hex'))
                    let hash_buffer = Buffer.from(SHA256(datawords).toString(), 'hex')
                    const hash = hash_buffer.readUInt32LE(0)
                    let interop_name = interopmethod[hash]
                    if (interop_name == null) {
                        interop_name = "- UNKNOWN Interop: " + data.toString('utf8')
                    }
                    out += `${inst} ${interop_name}\n`
                    ip += method_length-1 // -1 because ip always gets increased by one, whereas this is inclusive
                }
